...
const data = {
  ...
  "filePathRelative": "3.x/troubleshooting.md",
  "git": {
...
;// CONCATENATED MODULE: ./node_modules/vue-loader/dist/templateLoader.js??ruleSet[1].rules[1]!./node_modules/@vuepress/bundler-webpack/lib/build/ssr/vuepressLoader.js!./node_modules/vue-loader/dist/index.js??ruleSet[0].use[1]!./.vuepress/.temp/pages/3.x/troubleshooting.html.vue?vue&type=template&id=69c4bdf8
...
function ssrRender(_ctx, _push, _parent, _attrs) {
  ...
  _push(`<!--[--><h1 id="疑难解答" tabindex="-1"><a class="header-anchor" href="#疑难解答" aria-hidden="true">#</a> 疑难解答</h1><p>在微信公众平台开发的道路上，遍布着各种大大小小的坑，有的人掉坑里，几经折腾又爬出来了，然后拍拍屁股走人。然而坑还在那里，还会继续有后来人掉进去……</p><p>这，是我们不愿看到的。</p><p>所以在这里，我们将陆续将微信开发中可能遇到的各种疑难问题进行汇总，并给出对应的解决办法。一般情况下，这些问题都可以对号入座，轻松地解决。但也不排除特殊情况，这时候你遇到的问题与文中某一个症状一致，但文中所给的解决方案并不凑效，这种情况下就需要发挥你自己的智慧，去……折腾了……</p><p>我们期待这一版块为各位的开发带来便利，同时也希望各位本着开源、分享的精神对其进行补充和完善，将各种坑一一填小、填平，让微信开发变得不那么痛苦，甚至，变成一件快乐的事……</p><h1 id="一些服务器基本设施问题" tabindex="-1"><a class="header-anchor" href="#一些服务器基本设施问题" aria-hidden="true">#</a> 一些服务器基本设施问题：</h1><ul><li>时区不对， 使用命令 <code>date</code> 可以在服务器上查看当前时间，如果发现时区不对则需要修改时区：<a href="https://www.liberiangeek.net/2013/02/setting-the-correct-timezone-in-centos-and-ubuntu-servers-with-ntp/" target="_blank" rel="noopener noreferrer">Setting The Correct Timezone In CentOS And Ubuntu Servers With NTP`)
  _push((0,server_renderer_.ssrRenderComponent)(_component_OutboundLink, null, null, _parent))
  _push(`</a><ul><li>...</li></ul></li></ul><h2 id="curl-60-ssl-certificate-problem-unable-to-get-local-issuer-certificate" tabindex="-1"><a class="header-anchor" href="#curl-60-ssl-certificate-problem-unable-to-get-local-issuer-certificate" aria-hidden="true">#</a> curl: (60) SSL certificate problem: unable to get local issuer certificate</h2><p>这是 SSL 证书问题所致，在使用 SDK 调用微信支付等相关的操作时可能会遇到报 “SSL certificate problem: unable to get local issuer certificate” 的错误。</p><p>微信公众平台提供的文档中建议对部分较敏感的操作接口使用 https 协议进行访问，例如微信支付和红包等接口中涉及到操作商户资金的一些操作。 wechat SDK 遵循了官方建议，所以在调用这些接口时，除了按照官方文档设置操作证书文件外，还需要保证服务器正确安装了 CA 证书。</p><ol><li>下载 CA 证书</li></ol><p>你可以从 http://curl.haxx.se/ca/cacert.pem 下载 或者 使用<a href="https://pay.weixin.qq.com/wiki/doc/api/app.php?chapter=4_3" target="_blank" rel="noopener noreferrer">微信官方提供的证书`)
  _push((0,server_renderer_.ssrRenderComponent)(_component_OutboundLink, null, null, _parent))
  _push(`</a>中的 CA 证书 <code>rootca.pem</code> 也是同样的效果。</p><ol start="2"><li>在 <code>php.ini</code> 中配置 CA 证书</li></ol><p>只需要将上面下载好的 CA 证书放置到您的服务器上某个位置，然后修改 <code>php.ini</code> 的 <code>curl.cainfo</code> 为该路径（<strong>绝对路径！</strong>），重启 <code>php-fpm</code> 服务即可。</p><div class="language-text ext-text line-numbers-mode"><pre class="language-text"><code>curl.cainfo = /path/to/downloaded/cacert.pem
</code></pre><div class="line-numbers"><span class="line-number">1</span><br></div></div><blockquote><p>注意证书文件<strong>路径为绝对路径</strong>！以自己实际情况为准。</p></blockquote><p>其它修改 HTTP 类源文件的方式是不允许的。</p><h2 id="curl-error-56-sslread-return-error-9806" tabindex="-1"><a class="header-anchor" href="#curl-error-56-sslread-return-error-9806" aria-hidden="true">#</a> cURL error 56: SSLRead() return error -9806</h2><p>目前在 OSX 下，发现使用 HomeBrew 装的 PHP 7.0 有这个问题，解决方案是重新 brew 安装 PHP：</p><div class="language-bash ext-sh line-numbers-mode"><pre class="language-bash"><code>\$ brew <span class="token function">install</span> homebrew/php/php70 --with-homebrew-openssl --with-homebrew-curl --without-snmp -vvv
...
;
//# sourceMappingURL=1927.app.js.map